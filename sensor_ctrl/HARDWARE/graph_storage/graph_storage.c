...
*param buf 输出缓冲区
*retval 编码后的字节数
*note  存储格式：版本号(1B) + 节点数(1B) + 边数(1B) + 边列表(每边4B)
*       边列表格式：from(1B) + to(1B) + weight(1B) + shelf_id(1B)
*       只存储无向图的单向边（from < to），避免重复
*/
